...
png(filename = "edgestriangle1000.png", ##### Ir cambiando el nombre #####
...
# Como es el que comparamos solo CC no hay que tocar nada.
X <- data1$Valor
Y1 <- data1$Indice
Y2 <- data2$Indice
Y3 <- data3$Indice
...
plot(X, Y1, 
     type = "l",                    
     main = "Relació entre CC i la probabilitat (q) per a diferents H",  
     xlab = "Prob. q",                        
     ylab = "Nombre de CC",                       
     col = "blue",                          
     lwd = 2,
     ylim = c(min(c(Y1, Y2, Y3, Y4, Y5)), max(c(Y1, Y2, Y3, Y4, Y5))))                               # Ancho de la línea
